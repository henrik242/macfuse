...
static fusefs_recursive_lock* fusefs_recursive_lock_alloc_with_lock_group(lck_grp_t *lock_group) {
    struct _fusefs_recursive_lock *lock;

    if (lock_group == NULL)
        return 0;
	
    lock = (struct _fusefs_recursive_lock*) FUSE_OSMalloc(sizeof(struct _fusefs_recursive_lock),
														  fuse_malloc_tag);
    if (!lock)
        return NULL;
    ...
    if (lock->mutex) {
        ...
        FUSE_OSFree(lock,
					sizeof(struct _fusefs_recursive_lock),
					fuse_malloc_tag);
        lock = 0;
    }
